    ...
    try {
      ...
      result.meshes.forEach((mesh) => {
        ...
        if (mesh.name === "RevenuePL") {
          ...
          mesh.position.x -= 0.02; // Move right to anchor right edge (negative X appears right)
          ...
          mesh.scaling.x = 0.8; // 20% reduction in width
          mesh.position.x += 0.02; // Move left to anchor left edge (positive X appears left)
