        ...
        result.meshes.forEach((mesh, index) => {
          if (mesh.material && mesh.name !== "__root__") {
            ...
            if (sectionName === "Customer Relationships") {
              ...
              // Create label plane with correct aspect ratio matching the PNG file and mesh shape
              const labelPlane = MeshBuilder.CreatePlane("customerRelationshipsLabel", {
                width: size.x * 0.8,   // Slightly wider for the smaller Customer Relationships mesh
                height: size.z * 0.25  // Taller to match the narrower mesh proportions
              }, scene);
              
              // Position within the mesh boundaries, in upper portion
              labelPlane.position.x = center.x;
              labelPlane.position.y = center.y + size.y * 0.3; // Lower position to be within shape
              labelPlane.position.z = center.z + size.z * 0.2; // Move towards upper part of rectangle
