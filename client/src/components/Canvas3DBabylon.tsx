...
export const Canvas3DBabylon: React.FC<Canvas3DBabylonProps> = ({ canvas, isTransitioning }) => {
  ...
  useEffect(() => {
    ...
    // Load separate Revenue Streams GLB model
    SceneLoader.ImportMeshAsync("", "/models/", "BMC_blender_07_RevenueStreams_1754184188498.glb", scene).then((result) => {
      if (result.meshes.length > 0) {
        console.log(`✅ Revenue Streams model loaded with ${result.meshes.length} meshes`);
        
        const revenueRootMesh = result.meshes[0];
        
        // Position at center-right area based on diagram layout
        revenueRootMesh.position = new Vector3(5.5, 0.1, 4.5); // Bottom-right position
        
        // Rotate entire model 180 degrees clockwise around Y-axis when in orthographic mode to fix upside-down text
        if (isOrthographic) {
          revenueRootMesh.rotation = new Vector3(0, Math.PI, 0);
        } else {
        ...
        console.log(`📦 Revenue Streams model positioned at (5.5, 0.1, 4.5) with scale 8.0`);
        
        // Apply material and interactivity to each mesh
        result.meshes.forEach((mesh, index) => {
          if (mesh.material && mesh.name !== "__root__") {
            const sectionName = "Revenue Streams";
            const baseColor = new Color3(0.005, 0.005, 0.005); // Very Dark Black
            
            // Create TransformNode parent for individual manipulation
            const transformNode = new TransformNode(`revenueTransform_${index}`, scene);
            
            // Store the mesh's current local transform relative to revenueRootMesh
            const localPosition = mesh.position.clone();
            const localRotation = mesh.rotation.clone();
            const localScaling = mesh.scaling.clone();
            
            // Set TransformNode as child of revenueRootMesh to maintain hierarchy
            transformNode.parent = revenueRootMesh;
            transformNode.position = localPosition;
            transformNode.rotation = localRotation;
            transformNode.scaling = localScaling;
            ...
            // Create new semi-gloss black plastic material
            const sectionMaterial = new PBRMetallicRoughnessMaterial(`revenueSection_${index}`, scene);
            ...
            // Add floating label plane for Revenue Streams
            console.log(`🏷️ Creating floating label for ${sectionName} mesh (index ${index})`);
            ...
            // Create label plane with proper dimensions
            const labelWidth = size.x * 0.8; // Adjust based on mesh size
            const revenueStreamLabelHeight = (labelWidth * 0.25) * 1.5; // 50% taller to prevent squishing
            console.log(`Revenue Streams Label Dimensions: ${labelWidth} x ${revenueStreamLabelHeight}, Aspect Ratio: ${(labelWidth/revenueStreamLabelHeight).toFixed(2)}`);
            
            const labelPlane = MeshBuilder.CreatePlane("revenueStreamsLabel", {
              width: labelWidth,
              height: revenueStreamLabelHeight
            }, scene);
            ...
            const labelTexture = new Texture("/textures/Label_RevenueStreams_1754184188498.png", scene);
            labelTexture.hasAlpha = true;
            ...
            console.log(`✅ ${sectionName} label plane created`);
            ...
            // Create billboard label above this mesh but make it invisible
            const labelContainer = new Rectangle(`revenueLabel_${index}`);
            labelContainer.widthInPixels = 200;
            labelContainer.heightInPixels = 40;
            labelContainer.cornerRadius = 8;
            labelContainer.color = "white";
            ...
            const labelText = new TextBlock(`revenueLabelText_${index}`, sectionName);
            labelText.color = "white";
            labelText.fontSize = "14px";
            labelText.fontFamily = "Arial, sans-serif";
            labelText.fontWeight = "bold";
            ...
            // Position label higher above mesh top with billboard behavior
            const revenueBillboardHeight = 1.2;
            
            // Link label to 3D position with billboard behavior
            labelContainer.linkWithMesh(mesh);
            labelContainer.linkOffsetY = `-${revenueBillboardHeight * 50}px`;
            
            // Hide the label by making it invisible
            labelContainer.isVisible = false;
            
            // Create content panel for click events (initially hidden)
            const contentPanel = new Rectangle(`revenueContentPanel_${index}`);
            contentPanel.widthInPixels = 320;
            contentPanel.heightInPixels = 240;
            contentPanel.cornerRadius = 12;
            contentPanel.color = "white";
            ...
            // Create title text at top of panel
            const titleText = new TextBlock(`revenueTitleText_${index}`, sectionName);
            titleText.color = "black";
            titleText.fontSize = "14px";
            titleText.fontWeight = "bold";
            titleText.fontFamily = "Arial, sans-serif";
            ...
            // Create close button
            const closeButton = new TextBlock(`revenueCloseButton_${index}`, "X");
            closeButton.color = "grey";
            closeButton.fontSize = "16px";
            closeButton.fontWeight = "bold";
            closeButton.fontFamily = "Arial, sans-serif";
            ...
            // Create content text area (below title)
            const contentText = new TextBlock(`revenueContentText_${index}`, "");
            contentText.color = "black";
            ...
            contentPanel.linkWithMesh(mesh);
            contentPanel.linkOffsetY = `-${(revenueBillboardHeight + 2.8) * 50}px`;
            ...
            // Add standard hover and click interactions matching the main model pattern
            const updateMeshHoverEnter = () => {
              const brightBlueColor = new Color3(0.0, 0.3, 0.8);
              sectionMaterial.baseColor = brightBlueColor;
            };
            ...
            // Hover enter
            mesh.actionManager.registerAction(new ExecuteCodeAction(ActionManager.OnPointerOverTrigger, updateMeshHoverEnter));
            
            // Hover exit
            mesh.actionManager.registerAction(new ExecuteCodeAction(ActionManager.OnPointerOutTrigger, updateMeshHoverExit));
            
            // Click interactions (same pattern as main model)
            mesh.actionManager.registerAction(new ExecuteCodeAction(ActionManager.OnPickTrigger, () => {
              const isCurrentlyClicked = (mesh as any).isClicked;
              ...
              if (isCurrentlyClicked) {
                ...
                // Restore all objects to original heights and colors
                const savedHeights = getOriginalHeights();
                contentPanelsRef.current.forEach(({ mesh: otherMesh, material }) => {
                  const objSectionName = (otherMesh as any).bmcSectionName;
                ...
                // Flatten previously selected object
                const previouslySelectedObject = getSelectedObject();
                if (previouslySelectedObject && adjustBMCSection) {
                  adjustBMCSection(previouslySelectedObject, { height: 0.1 });
                }
                ...
                setSelectedObject(sectionName);
                (mesh as any).isClicked = true;
                sectionMaterial.baseColor = new Color3(0.0, 0.3, 0.8);
                ...
                // Show content panel with section content
                const sectionContent = getSectionContent(sectionName);
                if (contentText && sectionContent) {
                  contentText.text = sectionContent;
                  contentPanel.isVisible = true;
                }
              }
            ...
            console.log(`🎨 Revenue Streams Mesh ${index}: ${mesh.name || 'unnamed'} - Interactive setup complete`);
          }
        });
