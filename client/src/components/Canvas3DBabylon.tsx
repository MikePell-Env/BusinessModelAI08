...
export const Canvas3DBabylon: React.FC<Canvas3DBabylonProps> = ({ canvas, isTransitioning }) => {
  ...
  useEffect(() => {
    ...
    // 180-degree rotation around vertical Y-axis - flipping only X coordinates for left-right swap:
    // Key Activities (ORANGE) - X flipped from negative to positive (left to right)
    ...
    // Customer Relationships (YELLOW) - X flipped from positive to negative (right to left)
    ...
    // Key Resources (RED) - X flipped from negative to positive (left to right)
    ...
    // Customer Channels (CYAN) - X flipped from positive to negative (right to left)
