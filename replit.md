...
- **Comprehensive BMC State Management Architecture (January 2025 - REFACTORED)**: Completely unified selection system by eliminating competing state management approaches. BMC state manager now serves as the single source of truth for all selection behavior, with legacy Zustand store following for backward compatibility. Removed dual state conflicts that caused visual flashing, incorrect heights, and selection state loss during view transitions. All visual logic consolidated into single `applyBMCVisualState` function. Fixed crashes caused by references to deleted functions during refactoring process.
